...
module SpreadBase
  ...
  module SimpleAccessLayer
    ...
    class Table
      ...
      # index             can be higher than rows number - in this case, the space will be filled
      #                   with unstyled empty rows.
      #
      def insert_row( index, row_data )
        ...
        # Fill the row data with null values, if required
        #
        ( row_values.size ... @column_styles.size ).each do
          row_values << [ nil, nil ]
        end

        ( @data.size ... index ).each do
          @data << [ nil ] + [[ nil, nil ]] * row_values.size 
        end
      ...
      # index             can be higher than columns number - in this case, the space will be filled
      #                   with unstyled empty columns.
      #
      def insert_column( index, column_data )
        ...
        #
        ( column_values.size ... @data.size ).each do
          column_values << [ nil, nil ]
        end

        # Extends @data, if required - at this point, column_values can only be as long or longer
        # than @data
        #
        column_values.zip( @data ).each do | value, row_data |
          if row_data.nil?
            ...
            #
            row_data = [[ nil, nil ]] * @column_styles.size
      ...
      def set_cell_style( column_index, row_index, style )
        ...
        @data[ row_index][ column_index + 1 ][ 1 ] = style
      end
